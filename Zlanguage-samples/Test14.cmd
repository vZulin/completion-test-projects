...
cd "$root"
echo "Running: $root/bazel.cmd run //build:local_idea_ultimate_run_tests_build_target -- ${args[*]}"
exec /bin/bash "$root/bazel.cmd" run //build:local_idea_ultimate_run_tests_build_target -- "${args[@]}"
...
cd /d "%ROOT%"
echo Running: %ROOT%\bazel.cmd run //build:local_idea_ultimate_run_tests_build_target -- "--jvm_flag=-Dintellij.build.test.main.module=%MODULE%" "--jvm_flag=-D%TEST_PATTERN_PROP%=%TEST_PATTERN%"%BAZEL_EXTRA%
"%ROOT%\bazel.cmd" run //build:local_idea_ultimate_run_tests_build_target -- "--jvm_flag=-Dintellij.build.test.main.module=%MODULE%" "--jvm_flag=-D%TEST_PATTERN_PROP%=%TEST_PATTERN%"%BAZEL_EXTRA%
exit /b %ERRORLEVEL%
