 ...
 *
 * Covers: TC-36, TC-37, TC-38, TC-39, TC-67, TC-41, TC-42.
 *
...
/** TC-36: Accept completion with Enter key — inserts item. */
fun acceptWithEnter() {
    val userName = "Ann"
    @Suppress("unused")
    val userAge = 21
    // <caret> TC-36: Delete 'userName' below, type 'user', select 'userName' from list,
    //   press Enter; expect 'userName' inserted
    println(userName)
}

/** TC-37: Accept completion with Tab key — replaces current token. */
fun acceptWithTab() {
    @Suppress("unused")
    val userName = "Ann"
    val userAge = 21
    // <caret> TC-37: Delete 'userAge' below, type 'user', select 'userAge' from list,
    //   press Tab; expect token replaced with 'userAge'
    println(userAge)
}

/** TC-38: Replace selection with completion item. */
fun replaceSelection() {
    val userName = "Ann"
    @Suppress("UNUSED_VARIABLE")
    val x = "REPLACE_ME"
    // <caret> TC-38: Select "REPLACE_ME" above, invoke completion, choose 'userName';
    //   expect replacement
    println(userName)
}

/** TC-39: Commit by dot — accept completion and immediately chain dot access. */
fun commitByDot() {
...
fun caretAfterFunctionSelect() {
    // <caret> TC-41: Delete 'buildUser("Ann", 21)' below, type 'build', select 'buildUser'
